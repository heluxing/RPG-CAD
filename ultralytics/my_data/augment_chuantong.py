...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        # index_groups=[]
        # for i in indexes:
        #     index_groups.append(self.dataset.near_img_index[i])#[[2,3,4,5,6],[12,13,14,15,15]...]
        # label_groups=[]
        # for index_list in index_groups:
        #     label_groups.append([self.dataset.get_image_and_label(i) for i in index_list])
        ...
        for i in range(len(labels)):
            ...
            labels[i] = self._mix_transform(labels[i],yc=yc, xc=xc,r=r)
...
class Mosaic(BaseMixTransform):
    ...
    def _mix_transform(self, labels,**kwargs):
        ...
        return (
            self._mosaic3(labels,**kwargs) if self.n == 3 else self._mosaic4(labels,**kwargs) if self.n == 4 else self._mosaic9(labels,**kwargs)
    ...
    def _mosaic4(self, labels,**kwargs):
        ...
        s = self.imgsz
        # yc, xc = (int(random.uniform(-x, 2 * s + x)) for x in self.border)  # mosaic center x, y
        yc, xc = kwargs["yc"],kwargs["xc"]  # mosaic center x, y
        for i in range(4):
            labels_patch = labels if i == 0 else labels["mix_labels"][i - 1]
            # Load image
            img = labels_patch["img"]
