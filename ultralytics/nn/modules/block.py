...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, Conv3D
from .BDGA import BDGA
...
class C2f3D(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck3D(self.c, self.c, shortcut, g, k=((3,3, 3), (3,3, 3)), e=1.0) for _ in range(n))
...
class Bottleneck3D(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3,3, 3), e=0.5):
        """Initializes a bottleneck module with given input/output channels, shortcut option, group, kernels, and
        expansion.
        """
        super().__init__()
...
class BDA(nn.Module):
    def __init__(self, c_in, c_out, WH, wz, depth):  # c_in输入通道，在yaml文件里传进来，在task里处理一下
        ...
        self.BDGA = BDGA(dim=c_in, input_resolution=[WH, WH], depth=depth, num_heads=8, window_size=wz,
                         mlp_ratio=2.)
