...
class Conv3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(-1, self.depth, C, H, W)  # reshape to [batchsize, depth, c, H, W]
        # x = x.reshape(1, -1, C, H, W)  # for test
        x = x.permute(0, 2, 1, 3, 4)  #  [batchsize,  c, depth,H, W]
        x = self.act(self.bn(self.conv(x)))
        x = x.permute(0, 2, 1, 3, 4)  #  [batchsize, 12, c, H, W]
        C, H, W = x.shape[2], x.shape[3], x.shape[4]
        x = x.reshape(-1, C, H, W)  #  [batchsize*12, c, H, W]
        #
        return x
...
class Conv3D_total3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(x.shape)==4:
            C, H, W =x.shape[1], x.shape[2], x.shape[3]
            x = x.reshape(-1, self.depth, C, H, W)  #[batchsize, depth, c, H, W]
            # x = x.reshape(1, -1, C, H, W)  # for test
            x = x.permute(0, 2, 1, 3, 4)  # [batchsize,  c, depth,H, W]
