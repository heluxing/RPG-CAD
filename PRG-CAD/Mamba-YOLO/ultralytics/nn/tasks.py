...
from ultralytics.nn.modules.CRA import CRA
from ultralytics.nn.modules.transformer importBDA
            ...
            try:
                ...
                if 'Not implemented on the CPU' in str(
                    ...
                    m.stride = torch.tensor([s / x.shape[-2] for x in
                                             forward(torch.zeros(12, ch, s, s).to(torch.device('cuda')))])  # forward
                else:
                    raise e
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C3x,
            RepC3,
            SimpleStem, VisionClueMerge, VSSBlock, XSSBlock,CRA,BDA
        }:
            c1, c2 = ch[f], args[0]
