...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        for sub_dir in files:
            sub_dir_imgs=[]
            sub_dir_imgs.extend(sorted(glob.glob(os.path.join(sub_dir, "*.*"))))
            # Define files as images or videos
            images, videos = [], []
            for f in sub_dir_imgs:
                suffix = f.split(".")[-1].lower()  # Get file extension without the dot and lowercase
                if suffix in IMG_FORMATS:
                    images.append(f)
                elif suffix in VID_FORMATS:
                    videos.append(f)
            ni, nv = ni+len(images), len(videos)
            all_imgs.append(images)
        self.files = all_imgs + videos
        self.nf = len(all_imgs)  # number of files
    ...
    def __next__(self):
        ...
        while len(imgs) < self.bs:
            # self.nf=21
            if self.count >= self.nf:  # end of file list 应该是个列表才对，根据count作为索引
                if imgs:
