...
from src.data import BaseCollateFunction
...
class MyDataset(Dataset):
    ...
    def __init__(
        ...
        super().__init__()
        default_data = {
            'train': '/home/HELUXING/Brain_project/data/sequence_dataset_len6_cls8/images/train_6',
            'val': '/home/HELUXING/Brain_project/data/sequence_dataset_len6_cls8/images/val_6',
            'test': '/home/HELUXING/Brain_project/data/sequence_dataset_len6_cls8/images/test',
            'names': {0: 'naoquexue', 1: 'luguguzhe', 2: 'naochuxie', 3: 'ruanhuazao', 4: 'A-naobaizhishusong',
                      5: 'toupixiezhong', 6: 'zhanwei', 7: 'naowaijiye', 8: 'zhongxianyiwei', 9: 'lugubingbian',
                      10: 'naoweisuo', 11: 'gaihuazao'},
            'yaml_file': '/home/HELUXING/Brain_project/data/sequence_dataset_len6_cls8/sequence_dataset_8cls.yaml',
            'nc': 12}
    ...
    def collate_fn_DFINE(batch):
        ...
        for item in batch:
            ...
            for i in range(6):
                ...
                # 获取最末级子文件夹名
                last_folder = os.path.basename(os.path.dirname(img_path))
                filename_without_ext = os.path.splitext(os.path.basename(img_path))[0]
                # 分割文件名和扩展名
                image_id = f"{last_folder}__{filename_without_ext}"
                # 处理boxes
                if len(img_boxes) == 0:
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(
        ...
        batch_size=batch_size,
        shuffle=True,
        collate_fn=getattr(train_dataset, "collate_fn_DFINE", None),
        num_workers=4
    )
    # 创建数据集
    ...
    for batch_idx, (labels) in enumerate(val_dataloader):
        ...
        # print(f"图像尺寸: {images.shape}")  # 应为 [4, 1, 6, 512, 512]
        print(f"标签结构: {len(labels)} 个患者，每个包含6个切片的标注")
