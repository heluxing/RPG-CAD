    ...
    def load_state_dict(self, state):
        ...
        for k, v in self.__dict__.items():
            if hasattr(v, 'load_state_dict') and k in state:
                ...
                # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ 只加载权重，其它的都不加载：criterion，lr_scheduler等
                # if k == "model":
                #     model_dict = v.state_dict()
                #     pretrained_dict = state[k]
                #     matched_dict = {}
                #     mismatched_keys = []
                #     for k1, v1 in pretrained_dict.items():
                #         if k1 in model_dict:
                #             if model_dict[k1].shape == v1.shape:
                #                 matched_dict[k1] = v1
                #             else:
                #                 mismatched_keys.append((k1, model_dict[k1].shape, v1.shape))
                #         else:
                #             mismatched_keys.append((k1, "Key not in model", v1.shape))
                    # 加载匹配参数
                    # v.load_state_dict(matched_dict, strict=False)
                    # 输出详细报告
                    # print(f"权重加载完成: {len(matched_dict)}/{len(pretrained_dict)} 参数加载成功")
