...
if __name__ == "__main__":
    ...
    #   map_mode为0代表整个map计算流程，包括获得预测结果、计算VOC_map。
    #   map_mode为1代表仅仅获得预测结果。
    #   map_mode为2代表仅仅获得真实框。
    #   map_mode为3代表仅仅计算VOC_map。
    #   map_mode为4代表利用COCO工具箱计算当前数据集的0.50:0.95map。需要获得预测结果、获得真实框后并安装pycocotools才行
    #-------------------------------------------------------------------------------------------------------------------#
    map_mode        = 4 #先使用2生成GT，只需要运行一次，后面修改模型路径然后使用1生成预测结果，再使用3和4.
    #--------------------------------------------------------------------------------------#
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        retinanet = Retinanet(confidence = confidence, nms_iou = nms_iou)
        print("Load model done.")

        print("Get predict result.")
        # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
        test_img_path = "sequence_dataset—path/images/test"
        for sub_sequence_dir in tqdm(os.listdir(test_img_path)):
            retinanet.get_map_txt(sub_sequence_dir, class_names, map_out_path)
    ...
    classes_dic = {"Ischemia": "class_0",
                   "Hemorrhage": "class_1",
                   "Encephalomalacia": "class_2",
                   "Leukoaraiosis": "class_3",
                   "Scalphematoma": "class_4",
                   "Masseffect": "class_5",
                   "Extracerebral fluid collection": "class_6",
                   "Midlineshift": "class_7",
                   }
    if  map_mode == 2:
        print("Get ground truth result.")
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/" + image_id + ".txt"), "w") as new_f:
                root = ET.parse(os.path.join("data/voc_total/Annotations", image_id + ".xml")).getroot()
                for obj in root.findall('object'):
                    ...
                    if classes_dic[obj_name] not in class_names:
                        print(classes_dic[obj_name])
                        continue
                    ...
                    if difficult_flag:
                        new_f.write("%s %s %s %s %s difficult\n" % (classes_dic[obj_name], left, top, right, bottom))
                    else:
