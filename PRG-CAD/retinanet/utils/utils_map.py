...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path='./map_out'):
    # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$测试集val_6改为test
    # GT_PATH             = r"/home/HELUXING/Brain_project/retinanet-pytorch/ground-truth/val_6"
    GT_PATH = r"/home/HELUXING/Brain_project/code/retinanet-pytorch-3D/ground-truth/test"
    DR_PATH = os.path.join(path, 'detection-results')
    IMG_PATH = os.path.join(path, 'images-optional')
    TEMP_FILES_PATH = os.path.join(path, '.temp_files')  # 存放GT json文件
    ...
    # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    for sub_dir in os.listdir(GT_PATH):
        sub_dir_path = os.path.join(GT_PATH, sub_dir)
        ground_truth_files_list = glob.glob(sub_dir_path + '/*.txt')
        # ground_truth_files_list=os.listdir(sub_dir_path)
        if len(ground_truth_files_list) == 0:
            error("Error: No ground-truth files found!")
        for txt_file in ground_truth_files_list:
            file_id = txt_file.split(".txt", 1)[0]
            file_id = os.path.basename(os.path.normpath(file_id))
            temp_path = os.path.join(DR_PATH, sub_dir, file_id + ".txt")  # 找到对应的检测结果txt,只是用于核对有没有对应的
            ...
            os.makedirs(os.path.join(TEMP_FILES_PATH, sub_dir), exist_ok=True)
            subsenqunce_gt_json_dir = os.path.join(TEMP_FILES_PATH, sub_dir)
            with open(subsenqunce_gt_json_dir + "/" + file_id + "_ground_truth.json", 'w') as outfile:
                json.dump(bounding_boxes,
                          outfile)  # 写入到json文件里：[{"class_name": "naoquexue", "bbox": "232 228 251 253", "used": false},
                # {"class_name": "naoquexue", "bbox": "208 255 225 272", "used": false}]
    ...
    # dr_files_list.sort()
    # $$$$$$$$$$$$$$$$$$$$$$$$$$每个预测文件一次遍历，依次遍历每个类，每个类都只生成一个json
    for class_index, class_name in enumerate(gt_classes):
        bounding_boxes = []
        for sub_dir_dr in os.listdir(DR_PATH):
            dr_files_list = glob.glob(os.path.join(DR_PATH, sub_dir_dr) + '/*.txt')
            for txt_file in dr_files_list:
                # file_id = txt_file.split(".txt",1)[0]
                dir_name = os.path.basename(os.path.dirname(txt_file))
                from pathlib import Path
                file_stem = Path(txt_file).stem
                file_id = os.path.join(dir_name, file_stem)
                # file_id = os.path.basename(os.path.normpath(file_id))
                # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
                # temp_path = os.path.join(GT_PATH, sub_dir_dr,(file_id + ".txt"))
                # if class_index == 0:
                #     if not os.path.exists(temp_path):
                #         error_msg = "Error. File not found: {}\n".format(temp_path)
                #         error(error_msg)
...
def preprocess_dr(dr_path, class_names):
    results = []
    for dir in os.listdir(dr_path):
        image_ids = os.listdir(os.path.join(dr_path, dir))

        for image_id in image_ids:
            lines_list = file_lines_to_list(os.path.join(dr_path, dir, image_id))
            image_id = os.path.splitext(image_id)[0]
            for line in lines_list:
                line_split = line.split()
