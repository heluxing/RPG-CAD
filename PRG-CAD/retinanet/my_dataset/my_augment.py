...
from my_dataset.utils_mydataSet import LOGGER, colorstr
from my_dataset.utils_mydataSet.checks import check_version
from my_dataset.utils_mydataSet.instance import Instances
...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        mix_labels_sequence = [self.dataset.get_image_and_label(i) for i in indexes]  # 之前的代码
        # seed=random.randint(0, 65535)#对于同一张图片的前后帧所有的操作都一样一致，大家都用同一个随机种子
        # 用于马赛克增强随机中心点坐标
        s = self.imgsz
        ...
        resized_w_list=[]
        for i in range(4):
            resized_h_list.append(s)
            resized_w_list.append(s)
            # scale_random = random.uniform(1, 2)  # 仿射变换放大倍数1-2倍，这里设置了随机，仿射变换里就没有随机了

            # scale_random = random.uniform(0.5, 1.5)  # 仿射变换放大倍数1-2倍，这里设置了随机，仿射变换里就没有随机了
            #
            # affine_transform = RandomPerspective(
            #     degrees=0,
            #     translate=0,
            #     scale=scale_random,  # 缩放倍数
            #     shear=0,
            #     perspective=0,
            #     pre_transform=None
            # )
            # affine_transform_list.append(affine_transform)
            # resized_h_list.append(int(s * scale_random))
            # resized_w_list.append(int(s * scale_random))



        # 图1
        xc_up = max(int(random.uniform(int(s // 2), int(s // 2 * 1.5))), s - resized_w_list[0] // 2)  # 图1的左下角点
        yc = int(random.uniform(int(s // 2), resized_h_list[0]))  # 图1的左下角点
        y_down1 = int(random.uniform(0, resized_h_list[0] - yc))  # 图1的下移距离
        # y_down1 = int(random.uniform(s//10 if s//10<resized_h_list[0] - yc else 0, resized_h_list[0] - yc))  # 图1的下移距离
        # 图2
        xc_buttom = max(int(random.uniform(s // 2, s // 2 * 1.5)), s - resized_w_list[1]// 2)  # 图2的左上角点
        h_img2 = min(2 * s - yc, resized_h_list[1])
        y_down2 = int(random.uniform(0, resized_h_list[1] - h_img2))
        # y_down2 = int(random.uniform(s//10 if s//10<resized_h_list[1] - h_img2 else 0, resized_h_list[1] - h_img2))
        # 图3
        w_img3 = min(xc_up, resized_w_list[2] // 2)  # 这里是截取的一半的宽
        h_img3 = min(yc, resized_h_list[2])
        y_down3 = int(random.uniform(0, resized_h_list[2] - h_img3))
        x_left3 = int(random.uniform(0, resized_w_list[2] // 2 - w_img3))
        # y_down3 = int(random.uniform(s//10 if s//10<resized_h_list[2] - h_img3 else 0, resized_h_list[2] - h_img3))
        # x_left3 = int(random.uniform(s//10 if s//10<resized_w_list[2] // 2 - w_img3 else 0, resized_w_list[2] // 2 - w_img3))
        # 图4
        w_img4 = min(xc_buttom, resized_w_list[3] // 2)  # 这里是截取的一半的宽度
        h_img4 = min(2 * s - yc, resized_h_list[3])
        y_down4 = int(random.uniform(0, resized_h_list[3] - h_img4))
        x_left4 = int(random.uniform(0, resized_w_list[3] // 2 - w_img4))
        # y_down4 = int(random.uniform(s//10 if s//10<resized_h_list[3] - h_img4 else 0, resized_h_list[3] - h_img4))
        # x_left4 = int(random.uniform(s//10 if s//10<resized_w_list[3] // 2 - w_img4 else 0, resized_w_list[3] // 2 - w_img4))
        #
        random_parameter_list=[xc_up,yc ,y_down1,xc_buttom,h_img2,y_down2,w_img3,h_img3,y_down3,x_left3,w_img4,h_img4,y_down4,x_left4,resized_h_list]
        # 用于mixup
        r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
        for i in range(len(labels)):
            mix_labels = [mix_labels_sequence[j][i] for j in range(len(mix_labels_sequence))]
            if self.pre_transform is not None: # 这里不走
                for i, data in enumerate(mix_labels):
                    mix_labels[i] = self.pre_transform(data)

            # 先做放大处理
            # mix_labels[0] = affine_transform_list[1](mix_labels[0])
            # mix_labels[1] = affine_transform_list[2](mix_labels[1])
            # mix_labels[2] = affine_transform_list[3](mix_labels[2])
            # labels[i]=affine_transform_list[0](labels[i])


            labels[i]["mix_labels"] = mix_labels

            # Update cls and texts
            labels[i] = self._update_label_text(labels[i])
            # labels[i]是每一层，各层所用的尺寸变化应该一致
            labels[i] = self._mix_transform(labels[i], random_parameter_list=random_parameter_list, r=r)
            labels[i].pop("mix_labels", None)
        return labels

    # 混合的时候涉及随机的中心点，5层的应该都一样，所以设置一个随机种子，这个随机种子设置为中间那层
    def _mix_transform(self, labels, **kwargs):
        """
...
class Mosaic(BaseMixTransform):
    ...
    def get_indexes(self, buffer=True):
        ...
        """
        # random.seed(random.randint(0, 65536))
    ...
    def _mix_transform(self, labels, **kwargs):
        ...
        return (
            self._mosaic3(labels, **kwargs) if self.n == 3 else self._mosaic4(labels,
                                                                              **kwargs) if self.n == 4 else self._mosaic9(
                labels, **kwargs)
        )  # This code is modified for mosaic3 method.

    def _mosaic3(self, labels, **kwargs):
        """
    ...
    def _mosaic4(self, labels, **kwargs):
        """
        ...
        mosaic_labels = []
        s = self.imgsz
        # yc, xc_up = (int(random.uniform(int(-x), int(-x * 1.5))) for x in self.border)  # 图1的左下角点
        ...
        for i in range(4):
            ...
            #     degrees=0,
            #     translate=0,
            #     scale=1, # 缩放倍数：0.5-1.5
            #     shear=0,
            #     perspective=0,
            #     pre_transform=None
            # )
            # labels_patch=affine_transform(labels_patch)
            # Load image

            random_parameter_list = kwargs["random_parameter_list"]
            xc_up, yc, y_down1, xc_buttom, h_img2, y_down2, w_img3, h_img3, y_down3, x_left3, w_img4, h_img4, y_down4, x_left4 ,resized_h_list= random_parameter_list
            img = labels_patch["img"]
            h, w = labels_patch.pop("resized_shape")

            # Place img in img4
            if i == 0:  # top left
                # h, w = resized_h_list[i], resized_h_list[i]
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                # 这里的w、h在第一图中是缩放后的图片尺寸
                # xc_up = max(xc_up_random, s - w // 2)  # 图1的左下角点
                # yc = int(random.uniform(int(s // 2), h))  # 图1的左下角点,这里h不要超过1024，最好最大设置为512*1.5
                # y_down1 = int(random.uniform(0, h - yc))
                x1a, y1a, x2a, y2a = xc_up, 0, 2 * s - xc_up, yc  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = xc_up + w // 2 - s, y_down1, w // 2 + s - xc_up, y_down1 + yc  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # top right
                # xc_buttom = max(xc_buttom_random, s - w // 2)  # 图2的左上角点
                # h_img = int(random.uniform(s // 2, min(s * 2 - yc, h)))
                # h_img = min(2 * s - yc, h)
                # 随机生成一个上下移动的量
                # y_down = int(random.uniform(0, h - h_img))
                x1a, y1a, x2a, y2a = xc_buttom, yc, 2 * s - xc_buttom, yc + h_img2  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = xc_buttom + w // 2 - s, y_down2, w // 2 + s - xc_buttom, y_down2 + h_img2  # xmin, ymin, xmax, ymax (small image)
            elif i == 2:  # bottom left
                # w_img = min(xc_up, w // 2)  # 这里是截取的一半的宽
                # h_img = min(yc, h)
                # 随机生成一个上下移动的量
                # y_down = int(random.uniform(0, h - h_img))
                # x_left = int(random.uniform(0, w // 2 - w_img))
                x1a_l, y1a_l, x2a_l, y2a_l = xc_up - w_img3, 0, xc_up, h_img3  # xmin, ymin, xmax, ymax (large image)
                x1a_r, y1a_r, x2a_r, y2a_r = 2 * s - xc_up, 0, 2 * s - xc_up + w_img3, h_img3  # xmin, ymin, xmax, ymax (large image)
                # 小图要贴上去，不能512的大小贴，因为随机中心点，这里裁去了左上角区域
                x1b_l, y1b_l, x2b_l, y2b_l = w // 2 - w_img3 - x_left3, y_down3, w // 2 - x_left3, y_down3 + h_img3  # xmin, ymin, xmax, ymax (small image left)
                x1b_r, y1b_r, x2b_r, y2b_r = w // 2 + x_left3, y_down3, w // 2 + x_left3 + w_img3, y_down3 + h_img3  # xmin, ymin, xmax, ymax (small image right)
            elif i == 3:  # bottom right
                # w_img = min(xc_buttom, w // 2)  # 这里是截取的一半的宽度
                # h_img = min(2 * s - yc, h)
                # 随机生成一个上下移动的量
                # y_down = int(random.uniform(0, h - h_img))
                # x_left = int(random.uniform(0, w // 2 - w_img))
                x1a_l, y1a_l, x2a_l, y2a_l = xc_buttom - w_img4, yc, xc_buttom, yc + h_img4  # xmin, ymin, xmax, ymax (large image)
                x1a_r, y1a_r, x2a_r, y2a_r = 2 * s - xc_buttom, yc, 2 * s - xc_buttom + w_img4, yc + h_img4  # xmin, ymin, xmax, ymax (large image)
                x1b_l, y1b_l, x2b_l, y2b_l = w // 2 - w_img4 - x_left4, y_down4, w // 2 - x_left4, y_down4 + h_img4  # xmin, ymin, xmax, ymax (small image left)
                x1b_r, y1b_r, x2b_r, y2b_r = w // 2 + x_left4, y_down4, w // 2 + x_left4 + w_img4, y_down4 + h_img4  # xmin, ymin, xmax, ymax (small image right)
            if i < 2:
                img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
                padw = x1a - x1b
                padh = y1a - y1b
                # 图片已经贴进去了，这里对标签进行处理。只是对框的坐标进行调整，是对于大图的坐标
                labels_patch = self._update_labels_with_clip(labels_patch, padw, padh, x1b, x2b, y1b, y2b, img4)

                mosaic_labels.append(labels_patch)
            else:
                img4[y1a_l:y2a_l, x1a_l:x2a_l] = img[y1b_l:y2b_l, x1b_l:x2b_l]  # img4[ymin:ymax, xmin:xmax]
                img4[y1a_r:y2a_r, x1a_r:x2a_r] = img[y1b_r:y2b_r, x1b_r:x2b_r]  # img4[ymin:ymax, xmin:xmax]
                padw_l = x1a_l - x1b_l
                padw_r = x1a_r - x1b_r
                padh_l = y1a_l - y1b_l
                padh_r = y1a_r - y1b_r
                labels_patch1 = deepcopy(labels_patch)
                labels_patch2 = deepcopy(labels_patch)
                labels_patch1 = self._update_labels_with_clip(labels_patch1, padw_l, padh_l, x1b_l, x2b_l, y1b_l, y2b_l,
                                                              img4)
                mosaic_labels.append(labels_patch1)
                labels_patch2 = self._update_labels_with_clip(labels_patch2, padw_r, padh_r, x1b_r, x2b_r, y1b_r, y2b_r,
                                                              img4)
                mosaic_labels.append(labels_patch2)

        # 到这里，只完成了框的位置偏移，因为图片有部分被裁剪了，有些框的坐标超过1024还没有改到
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = img4
        ###########################################################
        ###############后期实验取图片#####################
        # _draw_box(img4, final_labels)
        ###########################################################
        ###############后期实验取图片#####################
        return final_labels

    def _mosaic9(self, labels, **kwargs):
        """
    ...
    @staticmethod
    def _update_labels_with_clip(labels, padw, padh, x0, x1, y0, y1, img4):
        """
        Updates label coordinates with padding values.
        ...
        nh, nw = labels["img"].shape[:2]
        labels["instances"].convert_bbox(format="xyxy")
        labels["instances"].denormalize(nw, nh)  # 把box的xyxy从相对转为绝对
        # 不使用自带的clip了，用自己的
        labels["instances"].bboxes[:, [0, 2]] = labels["instances"].bboxes[:, [0, 2]].clip(x0, x1)
        labels["instances"].bboxes[:, [1, 3]] = labels["instances"].bboxes[:, [1, 3]].clip(y0, y1)
        # labels["instances"].clip(w, h)  # 这里完成了对超出边界的框的裁剪，只保留wh内部的。
        labels["instances"].add_padding(padw, padh)  # 这里进行框的偏移，移动裁剪区域的宽度和高度，这时候有可能框的坐标已经超出512了，但还是先保留不动它
        return labels
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border, random_list):
        """
        ...
        # random.seed(seed)
        P[2, 0] = random_list[0]  # x perspective (about y)
        P[2, 1] = random_list[1]  # y perspective (about x)
        ...
        T[0, 2] = random_list[6]  # x translation (pixels)
        T[1, 2] = random_list[7]  # y translation (pixels)
    ...
    def __call__(self, labels):
        ...
        """
        if self.pre_transform and "mosaic_border" not in labels[0]:
            labels = self.pre_transform(labels)

        # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
        # 仿射变换里用到8个随机数，这里设置随机种子传入
        # seed =random.randint(0, 65532)
        img = labels[0]["img"]
        border = labels[0].get("mosaic_border", self.border)
        self.size = img.shape[1] + border[1] * 2, img.shape[0] + border[0] * 2  # w, h
        P1 = random.uniform(-self.perspective, self.perspective)  # x perspective (about y)
        P2 = random.uniform(-self.perspective, self.perspective)  # y perspective (about x)
        a1 = random.uniform(-self.degrees, self.degrees)
        scale_rand = random.uniform(1 - self.scale, 1 + self.scale)
        S1 = math.tan(random.uniform(-self.shear, self.shear) * math.pi / 180)  # x shear (deg)
        S2 = math.tan(random.uniform(-self.shear, self.shear) * math.pi / 180)  # y shear (deg)
        T1 = random.uniform(0.5 - self.translate, 0.5 + self.translate) * self.size[0]  # x translation (pixels)
        T2 = random.uniform(0.5 - self.translate, 0.5 + self.translate) * self.size[1]  # y translation (pixels)
        random_list = [P1, P2, a1, scale_rand, S1, S2, T1, T2]
        for label in labels:
            ...
            img, M, scale = self.affine_transform(img, border, random_list)
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        """
        # seed =random.randint(0, 65532)
        random_list = [random.random(), random.random()]
        for label in labels:
            img = label["img"]
            instances = label.pop("instances")
            ...
            # Flip up-down
            # random.seed(seed)
            if self.direction == "vertical" and random_list[0] < self.p:
                img = np.flipud(img)
                instances.flipud(h)
            if self.direction == "horizontal" and random_list[1] < self.p:
                img = np.fliplr(img)
                instances.fliplr(w)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if labels is None:
            labels = {}
        for label in labels:
            img = label.get("img") if image is None else image
            ...
            if len(label):
                label = self._update_labels(label, ratio, dw, dh)
                label["img"] = img
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        # seed = [random.randint(0, 512) for i in range(2)]
        ...
        for label in labels:
            if self.contains_spatial:
                ...
                if len(cls):
                    im = label["img"]
                    label["instances"].convert_bbox("xywh")
                    ...
                    if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                        label["img"] = new["image"]
                        label["cls"] = np.array(new["class_labels"])
                        bboxes = np.array(new["bboxes"], dtype=np.float32)
                    label["instances"].update(bboxes=bboxes)
            else:
                label["img"] = self.transform(image=label["img"])["image"]  # transformed
